...
export default defineEventHandler(async (event) => {
  ...
  const predikatKey = `predikat_${tahunStr}`;
  ...
  try {
    ...
    else predikatColumn = ikkHasil.predikat_2024; // validasi tahun sudah dilakukan, pasti salah satu
    ...
    const data = await db.select({
      ...
      .from(ikkHasil)
      .where(eq(ikkHasil.instansi_kategori, 'Kabupaten/Kota'));
